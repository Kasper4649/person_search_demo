...
def parse_model_cfg(path):
    ...
    for line in lines:  # '[net]'
        if line.startswith('['):  # 这是 cfg 文件中一个层(块)的开始
            mdefs.append({})  # 添加一个字典
            mdefs[-1]['type'] = line[1:-1].rstrip()  # 把 cfg 的 [] 中的块名作为键 type 的值  <class 'list'>: [{'type': 'net'}]
            ...
            if 'anchors' in key:
                # key:'anchors' val: ' 10,13,  16,30,  33,23,  30,61,  62,45,  59,119,  116,90,  156,198,  373,
                # 326' val.split(','): [' 10', '13', '  16', '30', '  33', '23', '  30', '61', '  62', '45', '  59',
                # '119', '  116', '90', '  156', '198', '  373', '326'] [float(x) for x in val.split(',')]: [10.0,
                # 13.0, 16.0, 30.0, 33.0, 23.0, 30.0, 61.0, 62.0, 45.0, 59.0, 119.0, 116.0, 90.0, 156.0, 198.0,
                # 373.0, 326.0] np.array([float(x) for x in val.split(',')]): shape:(18,
                # ) 将 val 以逗号划分元素，强制转成浮点数，形成了浮点数构成的列表， 再转成numpy格式数组reshape为(9, 2)，因为两个元素代表一个 anchor
